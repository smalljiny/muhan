...
//
// 관측 표면(UI 단언): 연결 상태 span(aria-label '연결 상태'), 이벤트 로그 ul(aria-label '이벤트 로그'),
// 명령 입력(label '명령')+'보내기' 버튼. 상태 도달은 web-first assertion(자동 재시도)으로 대기한다 — 고정 sleep 금지.
...
test('전송 왕복: 인증 접속(G1)·버전 협상(G2)·debug:echo 왕복(G3)', async ({ page }) => {
  ...
  await test.step('G1: 앱 로드 → __session 쿠키로 /game WS 접속 → ready 도달', async () => {
