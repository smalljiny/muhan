...
import { loadWorldGraph } from './worldGraph.js'
...
describe('loadWorldGraph — 실제 번들', () => {
  ...
  it('출구 엣지를 {name,targetRoomId,flags,key,timer:0}로 매핑하고 노드로 연결한다', () => {
    const graph = loadWorldGraph()
    ...
    expect(east).toEqual({ name: '동', targetRoomId: 69, flags: [0, 0, 0, 0], key: 0, timer: 0 })
  ...
  it('RoomNode에 monsters 필드가 없고 인메모리 필드만 갖는다(스코프 가드)', () => {
    ...
    expect(Object.keys(room50).sort()).toEqual(
      ['exits', 'items', 'longDesc', 'name', 'roomId', 'shortDesc'].sort(),
    )
    const pouch = room50.items.find((i) => i.name === '숨겨진 돈주머니')
