...
describe('createDevSeedAuthAdapter', () => {
  it('시드 쿠키를 DEV_SEED_ACCOUNT_ID로 검증하는 어댑터를 조립한다', () => {
    const adapter = createDevSeedAuthAdapter('dev-cookie', 'dev-account-1')

    expect(adapter.validateSessionCookie('dev-cookie')).toEqual({ accountId: 'dev-account-1' })
  })

  it('알 수 없는 쿠키는 null을 반환한다(유효 쿠키는 시드 쿠키 1개뿐)', () => {
    const adapter = createDevSeedAuthAdapter('dev-cookie', 'dev-account-1')

    expect(adapter.validateSessionCookie('other-cookie')).toBeNull()
  })

  it('시드 account에 고정 캐릭터 1개를 심는다', () => {
    const adapter = createDevSeedAuthAdapter('dev-cookie', 'dev-account-1')

    const chars = adapter.listCharacters('dev-account-1')
...
describe('createDevSeedAuthAdapterFromEnv', () => {
  it('env 값에서 시드 쿠키·계정을 읽어 어댑터를 조립한다', () => {
    const adapter = createDevSeedAuthAdapterFromEnv({
      DEV_SEED_COOKIE: 'env-cookie',
      DEV_SEED_ACCOUNT_ID: 'env-account',
    })

    expect(adapter.validateSessionCookie('env-cookie')).toEqual({ accountId: 'env-account' })
  })
