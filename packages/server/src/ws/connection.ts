 ...
 * 여기 대입한다(초기값 null — 게이트 통과 전이거나 배선 오류 시 null). `ready`·`heartbeat`·`account`는
 * 소켓 수명 동안 갱신되는 mutable 슬롯이라 `readonly`를 두지 않는다.
 */
...
export function createConnectionContext(): ConnectionContext {
  return { protocolVersion: PROTOCOL_VERSION, ready: false, heartbeat: null, account: null }
}
