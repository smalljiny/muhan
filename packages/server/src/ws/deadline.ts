 ...
 *
 * 단발 setTimeout 모델: `rearm`이 데드라인 타이머를 (재)무장하고, `deadlineMs` 안에 진행(상태 전이 또는
 * create 서브상태 전진)이 없으면 타이머가 발화해 소켓을 graceful `close`한다(하트비트의 terminate와 달리
 * 정상 종료 프레임을 보낸다). 진행이 있을 때마다 셸/FSM이 `rearm`을 불러 타이머를 새로 무장한다. 하트비트가
...
/** per-connection 데드라인 핸들. `rearm`은 데드라인을 (재)무장하고, `clear`는 idempotent 취소다. */
 ...
 *
 * `rearm`은 기존 타이머를 clear한 뒤 새 setTimeout을 무장한다(첫 무장·재무장 모두 동일 동작). 만료 시
