...
| 3 | **HP/MP 곡선 = `max = start + 성장률×(level−1)/2` 폐형.** class_stats[class]의 hpstart/mpstart/hp/mp가 정본. 805–808 재계산이 지배, 홀짝 증분(779–780)은 vestigial | 콘텐츠(곡선) / 형상(증분 잔재) |
...
| 13 | **데드코드 다수.** piety 재생 가속(595 주석), PBLESS thaco−3(대입 후라 무효), up_level 홀짝 증분(재계산에 덮임), bonus[35] 구표 주석 | 형상(비재현) |
...
> **형상 잔재(#3)**: `up_level`은 이 폐형 재계산 이전에 **홀짝 증분**(779–780: 홀수레벨 `hpmax+=hp`, 짝수 `mpmax+=mp`)을 먼저 수행한다. 그러나 `level%4==0`·`level==1`에서 805–808 재계산이 같은 값으로 **덮어쓴다**(증분 합 = 폐형과 수학적으로 일치 확인). `level%4≠0`에서는 791행 조기 `return`으로 재계산을 건너뛰고 증분만 남지만, 이 역시 폐형과 일치한다. 즉 **증분은 중복·불필요**하며, 유일한 관측 가능 효과는 L2 직후 mp가 일시적으로 폐형과 다르게 보일 수 있는 미세 드리프트뿐이다. 신규 스택은 폐형 하나만 구현한다.
...
| c | **up_level 홀짝 증분**(779–780) — 805–808 폐형 재계산에 덮여 무의미(값 일치) | 중복 | 폐형 하나로 통합 |
...
- **핵심 주장 spot-check**: hp/mp 폐형 `start+성장×(level−1)/2`(805–808, 증분과 값 일치 수동 검산 완료), piety 재생 주석(595행 확정), PBLESS thaco 대입 후 감산(1021 무효 확정), 사망 시 down_level 미호출·레벨 클램프(creature.c 128–139 확정), 종족 효과 3종(->race 전 파일 grep으로 스탯보정·장비크기·암시야 외 없음 확증), 승급 L100 리셋·L127 초인 고정치(command7.c 606·618 확정).
