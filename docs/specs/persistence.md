...
| `statusEffects` | `strictObject(...).partial().optional()` | 상태이상 영속 표현(선택) — poison/disease=`{until,interval}`, blind/silence/fear=`{until}`, 절대-틱 만료 |
...
| `gender` | `int().optional()` | 생성 인터뷰 성별 1=남/2=여 |
| `alignment` | `int()` | 생성 인터뷰 성향 1=선/2=악. 세 인터뷰 스칼라 중 유일한 **required** — 값역 제약 없음([character-flags.md](character-flags.md)) |
| `weapon` | `int().optional()` | 생성 인터뷰 주력 무기 1~5(도/검/봉/창/궁) |
| `schemaVersion` | `int()` | |

권위 인벤토리 배열 없음 — `object.owner={type:'character'}` 역참조로 파생. 게임 비밀번호는 담지 않는다(인증은 Firebase 소유). **E5 링크 필드**([`account-character.md`](account-character.md)): `accountId`(소유 계정 필수 FK), `status`(active/deleted soft-delete)·`deletedAt`, 생성 인터뷰 스칼라 `gender`/`weapon`/`alignment` 3종이 추가돼(당시 전부 optional — `alignment`는 이후 v6에서 required로 승격, 아래 **E6 P-flag 필드** 참조), 이 표는 E5 이후 5번째 스키마 `account.ts`와 함께 확장됐다. **E6 전투 필드**([`combat.md`](combat.md)): `hpCurrent`/`mpCurrent`/`level` 3종이 전투 resolver의 매-라운드 operand로 required 추가됐다(`gender`/`weapon`의 optional과 달리 required — 전투가 항상 값을 요구). 정의·시딩만 E6 소관이고 변이 의미론(레벨업·재생·exp)은 진행 루프(#81)가 얹는다. `experience`는 진행 루프(#81)가 level 동렬 required로 추가했고, `statusEffects`는 [combat.md](combat.md) 상태이상 DoT가 선택 필드로 추가했다(미지정=상태이상 없음, gender/weapon 선택 관례 — required면 기존 픽스처 파괴). **E6b-2 마법 필드**([`magic-progression.md`](magic-progression.md)): `spells`·`realm` 2종이 마법 계층(#85)의 required 영속 필드로, `buffs`는 선택 필드로 추가됐다. `spells`(uint8[16] 지식 비트마스크)·`realm`(realm[4] 누적경험치)은 학습·시전 게이트·realm 숙련 성장이 값을 요구하므로 required(experience 선례)이고, `buffs`(주문번호별 `{until}`)는 statusEffects 선례대로 선택(미지정=버프 없음)이다. **E6 P-flag 필드**([`character-flags.md`](character-flags.md)): `statusEffects`에 `silence`·`fear`가 `{until}`로 추가되고(blind 선례 — 주기 피해가 아니라 간격 없음), `alignment`가 optional에서 **required로 승격**됐다(`gender`/`weapon`은 optional 유지). alignment는 학습 게이트·전투 상태가 매 판정마다 값을 요구해 부재를 허용하면 소비 지점마다 폴백이 흩어지기 때문이다.

`schemaVersion`은 v6까지 증분됐고, load 직전 **stepwise 합성 체인**(`backfillCharacterV6∘V5∘V4∘V3∘V2`)이 구버전 문서를 승격한다: V2가 v1→v2(`seedVitals` class·level→`computeHpMax`/`computeMpMax`, `level=1` 고정), V3가 v2→v3(`experience` level 정합 시딩 `level<=1?0:neededExp(level-1)`), V4가 v3→v4(`schemaVersion` 스탬프만 — statusEffects는 선택 필드라 미시딩), V5가 v4→v5(`spells`=빈 비트마스크(16바이트 0)·`realm`=[0,0,0,0] 시딩, `buffs`는 선택 필드라 미시딩), V6가 v5→v6(`alignment` 중립 sentinel `0` 시딩 — `statusEffects.silence`/`fear`는 하위 선택 키라 미시딩, V4 선례). 각 스텝 가드·스탬프는 자기 리터럴 버전에 매인다(CURRENT 참조 금지 — silent 클로버 방지). write-back 없는 lazy 마이그레이션·멱등. 신규 문서는 생성 경로(`createCharacter`)에서 `CURRENT_CHARACTER_SCHEMA_VERSION`(현 6)·만피/만마·빈 spells·[0,0,0,0] realm으로 시딩한다.

**V6 시딩은 조건부이며 가드가 `Number.isInteger`다.** v5에서 alignment가 선택이었으므로 v5 문서 일부는 이미 1|2 실값을 갖는다 — V5의 무조건 시딩(v4엔 spells가 존재할 수 없었다)과 달리 값 보유 문서를 `0`으로 클로버하면 안 된다. 가드로 `=== undefined`를 쓰면 **Mongo가 null로 저장한 값이 통과해** 곧바로 `z.int()` parse를 깨뜨리고(V2의 `raw.class`·V3의 `raw.level`이 이미 같은 관용을 쓴다), `typeof === 'number'`를 쓰면 손상 문서의 `NaN`이 보존돼 parse가 hard throw하면서 **그 캐릭터가 영구히 로드 불가**가 된다(자가 치유 없음). `Number.isInteger`는 null·undefined·NaN·소수를 모두 "값이 아님"으로 보고 재시딩한다. 값역 위반(예 `5`)은 시딩 대상이 아니라 parse 에러로 드러나야 하므로 이 가드는 범위를 보지 않는다.

`characterRepository.parseCharacterDoc`이 소유하는 불변식은 하나다 — **backfill은 parse에 선행한다**. 스텝별 시딩 책임은 `characterBackfill.ts` 모듈 헤더가 정본이며 게이트 쪽에 재전사하지 않는다(버전이 오를 때마다 두 서사가 어긋난다).
...
- **`ObjectRepository`**(`objectRepository.ts`) — `IRepository<ObjectInstance>` 구현. `_id`가 Mongo `_id`로 그대로 매핑(별도 매핑 계층 없음). `findByOwner(owner: ObjectOwner)`가 `{'owner.type', 'owner.id'}` dot-notation 필터로 소유 집합을 조회하고 **`_id` 오름차순으로 정렬한다**. 이 정렬은 인벤 서수(`비법서 2 연마`)의 계약이다 — Mongo 자연 순서는 계약이 아니라 같은 인벤이 조회마다 다른 서수를 낼 수 있고, 서수는 플레이어가 관측하는 동작이다(오라클 대비 divergence 근거는 [`live-world-foundation.md`](live-world-foundation.md), 추적 [#142](https://github.com/smalljiny/muhan/issues/142)). `init()`이 `{'owner.type':1, 'owner.id':1, '_id':1}` 복합 인덱스를 생성(멱등) — `_id`를 뒤에 붙여 정렬을 인덱스가 직접 제공하게 한다(없으면 IXSCAN 뒤에 blocking in-memory SORT 스테이지가 붙는다). 구 2키 인덱스는 신 인덱스의 순수 prefix라 잉여지만 **부팅 경로에서 드롭하지 않는다** — 롤링 배포 중 구·신 버전이 번갈아 만들고 지우는 thrash, 부팅 임계 경로의 컬렉션 락, 오퍼레이터가 만든 진단 인덱스의 무단 삭제를 피하기 위해서다. 정리는 1회성 마이그레이션 소관이다. 경계 검증: `insert` 직전과 `findById`/`findByOwner` 직후 `objectSchema.parse`, `updateById`는 모듈 스코프 `objectSchema.partial()`로 부분 검증.
- **`CharacterRepository`**(`characterRepository.ts`) — `IRepository<Character>` 구현. 생성자가 `Db` + `ObjectRepository`(인벤토리 하이드레이션용)를 함께 주입받는다. `init()`이 `{name:1}` unique 인덱스를 생성(원작 FS 초성 샤딩 대체). `hydrateInventory(characterId)`가 `objects.findByOwner({type:'character', id: characterId})`로 인벤토리 뷰를 파생한다. `updateById`의 부분 검증 스키마는 **자기 모듈이 아니라 `characterSchemas.ts`**에서 온다(아래).
- **`characterSchemas.ts`** — `characterPatchSchema`(`characterSchema`를 `status.removeDefault()` 후 `.partial()`)의 단일 출처인 **의존 0 모듈**. 소비자가 둘이라 저장소 구현 모듈에서 분리했다 — 영속 경로(`CharacterRepository.updateById`)와 재접속 hydrate의 pending overlay가 **같은 인스턴스**로 검증해야 하는데([save-policy.md](save-policy.md) §미영속 스냅샷 조회), 후자는 `world/` 계층에 있다. 저장소 구현에 두면 `world/`가 `repo/` 구현을 런타임 의존하게 되고 이 서버에서 유일한 계층 역전 엣지가 된다(`world/`·`ws/`는 저장소를 구조적 타입으로만 받는다). `repo/types.ts`와 같은 지위다. `characterRepository.ts`가 재-export해 기존 import 경로를 보존한다. 재파생은 금지한다 — `removeDefault()` 같은 교정이 한쪽에만 반영되면 허용 키가 갈리고, 그 불일치는 예외도 로그도 남기지 않는다.
